...
if ($category) {
    // If a category is selected, fetch posts that match the category
    $stmt = $pdo->prepare("
...
ORDER BY (COALESCE(like_count.likes, 0) + COALESCE(comment_count.total_comments, 0)) DESC, posts.created_at DESC;
    ...
    // Создаем массив условий LIKE (ищем все слова в тексте)
    $searchConditions = [];
    foreach ($searchTerms as $term) {
        $searchConditions[] = "posts.content LIKE ?";
    }

    // Объединяем условия через AND, чтобы все слова обязательно были в посте
    $sqlCondition = implode(" AND ", $searchConditions);
    ...
    // Привязываем параметры (LIKE с % для частичного совпадения)
    foreach ($searchTerms as $index => $term) {
        $stmt->bindValue($index + 1, "%$term%", PDO::PARAM_STR);
    }
    ...
    $stmt = $pdo->prepare("SELECT posts.*, users.name, users.profile_image, users.email,  
       COALESCE(like_count.likes, 0) AS likes,
       COALESCE(comment_count.total_comments, 0) AS comment_count  
FROM posts 
JOIN users ON posts.user_id = users.id  
...
$comments_stmt = $pdo->prepare("SELECT comments.*, users.name ,users.email,users.profile_image
                                FROM comments 
...
<script src="https://cdn.jsdelivr.net/npm/@joeattardi/emoji-button@4.6.2/dist/index.min.js"></script>
	...
	<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6018149767817429"
...
<body>
    ...
    $avatarLetter = strtoupper(substr($post['email'], 0, 1)); // Get first letter of email
    ?>
    ...
    <small>
        ...
        ?>
           ...
           <a href="https://qur.kz/post_detail.php?post_id=<?= htmlspecialchars($post['id']); ?>" style="text-decoration:none;  display:block;color:black">      <!-- Post Content -->
    ...
    <audio controls>
        <source src="https://qur.kz/k/<?php echo htmlspecialchars($post['music']); ?>" type="audio/mpeg">
        Your browser does not support the audio element.
    </audio>
<?php endif; ?>
				 <!-- Display Post Image if Available -->
                <?php if ($post['image']): ?>
    <img src="https://qur.kz/k/<?php echo htmlspecialchars($post['image']); ?>" 
         class="user-post-img" 
         alt="Post Image" 
